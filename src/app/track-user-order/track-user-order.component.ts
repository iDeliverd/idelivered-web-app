...
@Component({
  selector: 'app-track-user-order',
  templateUrl: './track-user-order.component.html',
  styleUrls: ['./track-user-order.component.scss']
})
export class TrackUserOrderComponent implements OnInit, OnDestroy {
  ...
  getDetail() {
    ...
    this.http.postData(ApiUrl.orders.trackOrder, obj)
      .subscribe(response => {
        if (!!response && response.data) {
          if (response.data.orderHistory.length) {
            this.orderHistory = response.data.orderHistory[0];

            if (this.orderHistory && this.orderHistory.agent && this.orderHistory.agent.length) {
              this.location = this.orderHistory.agent[0];

              if (this.orderHistory['order_id'] == this.orderId) {
                this.trackLocation();
              }
            }
          }
  ...
  trackLocation() {
    ...
    /*************** Agent ***************/
    let agent = new google.maps.LatLng(self.orderHistory.latitude, self.orderHistory.longitude);
  ...
  getDestination(): Array<string> {
    if (this.self_pickup == 1) {
      ...
      // ['address'] && this.location['address'].length) {

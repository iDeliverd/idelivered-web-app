...
import { ViewSupplierDocumentComponent } from '../../shared/product/components/view-supplier-document/view-supplier-document.component';
...
export class FoodListingComponent implements OnInit, OnDestroy {
    ...
    onWindowScroll() {
        ...
        let deduction_offset = this.settings.selected_template == 0 ? -200 : 180; 

        for (let key in this.pagePosition) {
            this.pagePosition[key] = this.document.getElementById(key)
                ? this.pagePosition[key] < (this.document.getElementById(key).offsetTop) - deduction_offset ? this.document.getElementById(key).offsetTop - deduction_offset : this.pagePosition[key]
                : null;
        }

        const positions = Object.keys(this.pagePosition);
        const lastIndex = positions.length - 1;
        let deduction_pixels = this.settings.selected_template == 1 ? -60 : 0; // 620;
    ...
    ngOnInit() {
        ...
        if(this.settings.doc_rating_upload == 1){
        this.getSupplierRatingDocs();
        }
    }
    ...
    getDetails() {
        ...
        if (this.settings.branch_flow == 1 && this.branchId) {
            param_data['supplier_branch_id'] = (Number)(this.branchId);
            ...
            .subscribe(response => {
                if (!!response && response.data) {
                    ...
                    if (this.supplierDetail['supplier_image'].length) {
                        this.supplierDetail['supplier_image'].forEach(element => {
                            if (element.length > 0) {
                                this.supplierDetail['supp_image'] = element;
                            }
                        });
                    ...
                    if (!this.showBranches) {
                        ...
                        if (this.products.length == 0 || (this.products.length == 1 && !this.products[0].value.length)) {
                            this.no_data = true;
                        } else {
                            this.no_data = false;
                        }
    ...
    getSupplierCategoryList() {
        ...
        let api = ApiUrl.getSupplierCategoryList;
        if (this.settings?.show_supplier_categories_api_only == 1) {
            api = ApiUrl.getSupplierCategoryListOnly
            param_data['languageId'] = this.util.handler.languageId;
            this.http.postData(api, param_data)
                .subscribe(response => {
                    this.category_list = response.data.list;
                    if (this.category_list.length) {
                        this.selected_category_id = this.category_list[0].id
                    }
                })
        } else {
            api = ApiUrl.getSupplierCategoryList;
            this.http.getData(api, param_data)
                .subscribe(response => {
    ...
    selectCategory(cat, index) {
        if (this.settings?.show_supplier_categories_api_only == 1) {
            this.selected_category_id = cat.id;
        } else {
    ...
    setImageDesigns() {
        ...
        if (this.settings.selected_template == 5) {
            this.bannerImgHeight = "300px"
        }
        if (screen.width <= 600) {
            this.bannerImgHeight = "200px"
        }
    }
    ...
    todaySupplierTimings() {
        if (this.supplierDetail.timing && this.supplierDetail.timing.length) {
            ...
            if (this.settings.show_closed_by_time) {
                const tomorrowTimes = this.supplierDetail.timing.find((i) => i.week_id == today + 1);

                if (tomorrowTimes) {
                    let tomorrowStartTime = tomorrowTimes.start_time.split(':');
                    const tomorrowOpeningTime = moment().set('h', tomorrowStartTime[0]).set('m', tomorrowStartTime[1]).set('s', tomorrowStartTime[2]);
                    this.openingTomorrowAt = tomorrowOpeningTime.format('LT');
                }
            }
            ...
            if (this.settings.enable_supplier_config_closing_day == 1) {
                const closingSlotTimes = this.supplierDetail.timing.find((i) => i.close_week_id == today);

                if (closingSlotTimes) {
                    let endTime = todayTimes.end_time.split(':');
                    const openingTime = moment().set('h', 0).set('m', 0).set('s', 0);
                    let closeTime;
                    // if(!!todayTimes.close_week_id && todayTimes.close_week_id!=todayTimes.week_id) {
                    closeTime = moment().set('h', endTime[0]).set('m', endTime[1]).set('s', endTime[2]);
                    if (moment().isAfter(openingTime, 'm') && moment().isBefore(closeTime)) {
                        this.supplierDetail['todayTimes']['is_open'] = 1;
    ...
    categorySelect(index) {
        ...
        if (this.settings.selected_template == 1) {
            ...
            this.window.scrollTo(0, this.pagePosition[index] + 200);
        }
        // this.categoryIndex = index;
    ...
    createTableBooking(data) {
        if (data) {
            ...
            this.http.postData(ApiUrl.makeTableBookingRequest, table_booking_param).subscribe((res: any) => {
                if (res.status == 200) {
                    this.message.alert("success", this.translate.instant("Table has been booked!"));
    ...
    getSupplierRatingDocs() {
        ...
        this.http.postData(ApiUrl.getSupplierRatingDoc, param_data)
            .subscribe(response => {
                if (!!response && response.data) {
                        this.supplierRatingDetails = response.data[0];
                }
            });
    }
    ...
    onClickDocuments(documents) {
        const dialogRef = this.dialogService.open(ViewSupplierDocumentComponent, {
            width: '50%',
            style: { 'background-color': `${this.style.primaryColor} !important` },
            showHeader: false,
            transitionOptions: '600ms cubic-bezier(0.25, 0.8, 0.25, 1)',
    ...
    copyClipboardPhone (phone_no) {
        const selBox = document.createElement('textarea');
        selBox.style.position = 'fixed';
        selBox.style.left = '0';
        selBox.style.top = '0';
        selBox.style.opacity = '0';
        selBox.value = phone_no;

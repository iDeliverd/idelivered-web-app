...
export class SupplierListingComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.routeSubscription = this.route.queryParams.subscribe(params => {
      ...
      this.is_from_categories = params['is_from_categories'] ? parseInt(params['is_from_categories']) : 0;
  ...
  getSuppliers() {
    ...
    if (this.settings.enable_supplier_sorting_by_distance) {
      param_data.sort_by = 1;
    }
    ...
    this.http
      ...
      .subscribe(response => {
        if (!!response && response.data) {
          ...
          this.pagination.count = response.data.supplierList.length;
  ...
  getCategoryDetail(cat: any) {
    ...
    if (this.is_from_categories) {
      queryParams['is_from_categories'] = 1;
    }

...
export class OrderAttachmentsComponent implements OnInit {
  ...
  set orderData(order) {
    if (order) {
      ...
      this.setPrecsAudios(this.order);
    }
  }
  ...
  public prescription_audio_arr: any = [];
...
setPrecsAudios(order){

  if(order.order_video){
    var prescription_audio_obj = {url: ''}
    prescription_audio_obj.url = order.order_video
    this.prescription_audio_arr.push(prescription_audio_obj.url)
  }

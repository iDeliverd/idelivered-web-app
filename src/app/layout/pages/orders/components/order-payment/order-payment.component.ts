...
export class OrderPaymentComponent implements OnInit {
  ...
  set setOrder(order) {
    if (order) {
      this.order = order;
      this.order['amount'] = order.remaining_amount ? order.remaining_amount : 0;
    }
  }
  ...
  makePayment() {
    ...
    if ((this.paymentType == 1 && this.order.payment_type == 3) || this.order.payment_type == 1) {
      form_data['card_id'] = this.transaction['card_id']
      form_data['customer_payment_id'] = this.transaction.customer_payment_id;

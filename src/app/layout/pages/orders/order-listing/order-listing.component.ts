...
export class OrderListingComponent implements OnInit, OnDestroy {
  ...
  cancelOrder(order: any) {
    this.message.confirm(`${this.translate.instant('Cancel This')} ${this.translate.instant(`${this.localization.transform('order')}`)}`).then(data => {
      if (data.value) {
        let param_data = {
          languageId: this.util.handler.languageId,
          accessToken: this.user.getUserToken,
        ...
        if(this.settings.order_statuswise_deduction_charges && this.settings.order_statuswise_deduction_charges == 1){
          this.http.postData(ApiUrl.orders.cancelOrderOnStatuswiseDeduction, param_data)
          .subscribe(response => {
            if (!!response && response.data) {
        ...
        this.http.postData(ApiUrl.orders.cancelOrder, param_data)
          .subscribe(response => {
            if (!!response && response.data) {
              this.message.toast('success', `${this.translate.instant(`${this.localization.transform('order')}`)} ${this.translate.instant('Cancelled Successfully')}`);
              this.getOrders();
            }
          });
        }

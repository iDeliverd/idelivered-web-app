...
export class RatingComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscribeRoute();
    if(this.settings.enable_emoticon_rating) {
      this.initform();
    }
  }
  ...
  onSubmit() {
    ...
    if(this.settings.hide_agent_rating == 0 && this.settings.hide_product_rating == 0){
      if (this.rating === 0) {
        this.message.toast('warning', `${this.translate.instant('Please Rate')} ${this.type === 'PRODUCT' ? this.localization.transform('product') : (this.type === 'supplier' ? this.localization.transform('supplier') : this.localization.transform('agent'))}`);  
        return;
      }
    }
  ...
  moveTo() {
    switch (this.type) {
      ...
      case 'SUPPLIER':
        ...
        break;
      case 'agent':
        this.message.toast('success', `${this.translate.instant(`${this.localization.transform('agent')}`)} ${this.translate.instant('Rated Successfully')}`);
    }
  ...
  initform(){
    this.emog = this.rate.group({
      emo: ['',[Validators.required,Validators.min(0.2),Validators.max(10)]]
    })
  }

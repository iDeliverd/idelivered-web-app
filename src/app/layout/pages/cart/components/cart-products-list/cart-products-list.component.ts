...
export class CartProductsListComponent implements OnInit, OnDestroy {
  ...
  mins(product: any, index: any) {
    ...
    if (product.selectedQuantity > minQty) {
      ...
      this.message.confirm(`${this.translate.instant('Remove This'+' '+this.localization.transform('product'))} `, this.settings.hide_popup_text_service_will_remove_from_the_cart ? '' :`${this.translate.instant('This')} ${this.localization.transform('product')} ${this.translate.instant('Will Get Removed From The Cart')}.`,true)
        .then(result => {
          if (result.value) {
            if (this.settings.is_decimal_quantity_allowed == 1) {
              this.products[index].selectedQuantity = this.cartService.decimalRoundOff(this.products[index].selectedQuantity - this.decimalQuantityStep);
  ...
  removeFromCart(index: any, custom?: boolean, custId?: any) {
    this.message
      .confirm(`${this.translate.instant('Remove This'+' '+this.localization.transform('product'))}`, this.settings.hide_popup_text_service_will_remove_from_the_cart ? "" : `${this.translate.instant('This'+' '+this.localization.transform('product'))} ${this.translate.instant('Will Get Removed From The Cart')}.`, true)
      .then(data => {
        if (data.value) {
          if (custom) {

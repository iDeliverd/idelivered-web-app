...
@Component({
  selector: 'app-cart-suggetion',
  templateUrl: './cart-suggetion.component.html',
  styleUrls: ['./cart-suggetion.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class CartSuggetionComponent implements OnInit, OnChanges, OnDestroy {
  @Input() product: any;
  ...
  decreaseQuantity(): void {
    ...
    if (this.settings.enable_every_item_remove_alert) {
      this.message.confirm(`remove item from cart`, ``, true)
        .then(result => {
          if (result.value) {
            this.onMins.emit();
          }
  ...
  getAddonProductPrice(productPrice, addons: Array<any>): string {
    let amount = this.settings.remove_price_in_customisation == 1 ? 0 : parseFloat(productPrice);
  ...
  showSpecialInstruction(item, i?: number) {
    this.special_instructions = ((i > -1) ? (item.customization[i].special_instructions) : item.special_instructions) || '';
  ...
  onProcessSpecialInstruction(event) {
    for (var i in this.cartItems) {
      if ((Number)(this.cartItems[i].productId) == this.product.productId && !(this.product.customization && this.product.customization.length)) {
        this.cartItems[i]['special_instructions'] = event;
        break;
      }
      if ((Number)(this.cartItems[i].productId) == this.product.productId && this.product.customization && this.product.customization.length && this.custom_index > -1) {
        this.cartItems[i].customization[this.custom_index]['special_instructions'] = event;
        break;
      }
    }

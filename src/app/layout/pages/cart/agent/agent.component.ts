...
export class AgentComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.cartSubscription = this.util.getCart
      .subscribe(cart => {
        ...
        this.cart = cart && cart.length ? cart : (!!service ? [service] : []);
  ...
  createResponseModel() {
    ...
    this.agents.forEach(element => {
      if (!!service) {
        ...
        element['fixed_price'] = element['agent_service_price'] = element['agent_service_price'] = 0;
        element['description'] = '';
      }
    });
  }
  ...
  getDates(): any[] {
    this.datesArr = this.datesArr.filter(function (el) {
      let day = moment(el).endOf('day').toString();
  ...
  addToCart(selectedAgent: any) {
    ...
    if (!!service) {
      ...
      this.message.toast('warning', this.translate.instant(`Please Select ${this.localization.transform('product')}`));
      if (!this.settings.isCustomFlow)
        this.router.navigate(['/']);
      else
        this.navigateToCurrentFLow()
    }

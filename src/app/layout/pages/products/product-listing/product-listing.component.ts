...
export class ProductListingComponent implements OnInit, OnDestroy {
  ...
  suppliers: any[] = [];
  isShowSideFilters: boolean = true;
  body: StyleConstants;
    ...
    this.settingsSubscription = this.util.getSettings
      .subscribe((settings: AppSettings) => {
        if (settings) {
          ...
          if (this.settings.hide_product_filter == 1) {
            this.isShowSideFilters = false;
          }
    ...
    this.filterSubscription = this.util.getFilters
      .subscribe(filterData => {
        if (filterData) {
          ...
          if (this.settings.enable_global_search_items == 1) {
  ...
  filterProducts(is_page_change: boolean) {
    ...
    if (!this.subCategoryIds.length) {
      if (this.settings.isCustomFlow && this.categoryFlowId) {
        param_data['subCategoryId'] = [Number(this.categoryFlowId)];
      }
    }
    ...
    if (this.supplierIds) {
      param_data['supplier_ids'] = this.supplierIds.length ? this.supplierIds :
        ((this.supplierData && this.supplierData.id) ? [Number(this.supplierData.id)] : []);
    }
    ...
    if (this.settings.enable_global_search_items != 1) {
      this.http.postData(ApiUrl.filterProducts, param_data, false)
        .subscribe((response: any) => {
          ...
          if (!!response && response.data) {
            ...
            this.pagination.count = response.data.count || response.data.product.length;
      ...
      let new_params = {};
      new_params['latitude'] = param_data['latitude'];
      new_params['longitude'] = param_data['longitude'];
      new_params['search'] = param_data['product_name'];
      new_params['limit'] = param_data['limit'];
      new_params['skip'] = 0;
      this.http.postData(ApiUrl.globalSupplier, new_params, false)
        .subscribe((response: any) => {
          this.productList = [];
          if (!!response && response.data) {
            this.addProductsToGoogleDataLayer(response.data);
  ...
  onBookNow(service: any) {
    ...
    if ((this.settings.laundary_service_flow == 1 || this.settings.hideAgentList == 1) && this.settings.app_type == 8) {
      if (this.settings.enable_services_customize_flow == 1) {
        if (this.settings.laundary_service_flow == 0) {
          this.util.setCart([]);
          if (service.adds_on && service.adds_on.length) {
            service.adds_on.forEach(addOn => addOn.value.forEach(value => value.quantity = 0));
            service.selectedQuantity = 0;
            this.addProduct(service);
          } else {

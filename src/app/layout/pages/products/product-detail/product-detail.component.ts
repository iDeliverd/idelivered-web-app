...
export class ProductDetailComponent implements OnInit, OnDestroy {
        ...
        if (!!response && response.data) {
          ...
          if (product_id) {
            ...
            if (this.detail.discount) {
              const discountPercentage = parseFloat((((this.detail.display_price - this.detail.fixed_price) / this.detail.display_price)
                * 100).toFixed(2));
              this.detail.discount = Math.round(discountPercentage);
            }
  ...
  replaceAllValue(index) {
    if (this.settings.app_type == 1 && this.settings.enable_food_varients) {
      return;
    }
  ...
  increaseValue() {
    ...
    if (this.detail.variants && this.detail.variants.length) {
      if (!this.settings.enable_food_varients && !this.unitId || this.unitId == this.detail.id) {
  ...
  addTocart() {
    ...
    this.cartService.setCartType(this.detail.type || this.settings.app_type);
  }
  ...
  getPriceAfterConversion(detail) {
    var price = "0";
    if (detail) {
      price = (((Number)(detail.fixed_price) * this.selected_currency.conversion_rate)
        .toFixed(this.settings.price_decimal_length));
    }
    return price;
  }
  getDiscountPriceAfterConversion(detail) {
    var price = "0";
    if (detail) {
      price = (((Number)(detail.display_price) * this.selected_currency.conversion_rate)
        .toFixed(this.settings.price_decimal_length));
    }
    return price;
  }

...
export class CartScheduleTimeComponent implements OnInit {
  ...
  @Output() tableDetails = new EventEmitter<any>();
  ...
  ngOnInit() {
   ...
   if (this.settings.schedule_after_day){
  ...
  submit() {
    if (!!this.selectedSlot && !this.selectedSlot.time) {
      ...
      const selectedTime = new Date(this.selectedSlot.startTime).getHours();
      if (this.settings.disable_slot_selection_2_to_5) {
        const startTime = 14;
        const endTime = 17.5;
        if (selectedTime >= startTime && (selectedTime <= endTime)) {
          this.message.toast('warning', 'Time slot is not available. Please choose another time slot.');
          return;
        }
      }
  ...
  selectSlot(slot: any) {
    ...
    this.tableRequestData.date = this.date;
    this.tableRequestData.time = this.selectedSlot.time;
  }
  ...
  onTableSelection(event) {
    if (!!event) {
      event.delivery_slot = this.selectedSlot;
    }

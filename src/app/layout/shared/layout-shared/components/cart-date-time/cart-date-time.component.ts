...
import { AllSlots, Slots } from './../../../../../shared/models/slots.model';
...
export class CartDateTimeComponent implements OnInit, OnChanges, OnDestroy {
  ...
  getSlots() {
    ...
    if(this.settings.enable_booking_gap_two_hrs == 1){
      params['duration'] = Number(this.cart[0]['duration']/60);
      // params['price_type'] =  this.cart[0]['price_type']
    }

    if(this.settings.count_customized_service_duration == 1){
      if(this.cart[0]['adds_on'] && this.cart[0]['adds_on'].length){
        params['duration'] = this.cart[0]['adds_on'][0]['selectedCount'];
      } else {
        params['duration'] = this.cart[0]['selectedQuantity'];
      }
    }
        ...
        if (!!response && response.data) {
          ...
          if(this.settings.enable_booking_gap_two_hrs == 1){
            if (moment(this.date).format('YYYY-MM-DD') === moment().format('YYYY-MM-DD')) {
              slots = _.filter(slots, (slot: any) => moment(moment(this.date).format('YYYY-MM-DD') + ' ' + slot).isAfter(moment().add(2,"hours")));
            }
          }else{
            if (moment(this.date).format('YYYY-MM-DD') === moment().format('YYYY-MM-DD')) {
          ...
          if(this.settings.hide_slots_by_time == 1){
            this.allSlotData = new AllSlots();
            this.allSlotData.slots =  (Object.values(this.slotData) as any).flat();
            this.allSlotData.slots =  this.allSlotData.slots.map((slot:string)=>{
              let slotArr=slot.split(" ");
              let [time,ampm] = slotArr;
              slot =  time+" "+this.translate.instant(ampm);
              return slot;
            })
          }
          this.noData = true;
  ...
  submit() {
    if (!this.time) {
      this.message.toast("error", this.translate.instant("Please select the slot time"));
      return;
    }
  ...
  generateOrder() {
    ...
    if (this.settings.enable_custom_orderId == 1){
      obj["custom_order_id_check"] = true;
    }
    ...
    this.http.postData(ApiUrl.generateOrder, obj, false)
      .subscribe(response => {
        if (!!response && response.data) {
          ...
          this.util.setCart([]);
          setTimeout(() => {
            this.message.alert('success', this.translate.instant(this.localization.transform('order').toString()) +` ${this.translate.instant('Placed Successfully')}!`);
          }, 800);

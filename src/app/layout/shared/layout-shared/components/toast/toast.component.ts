...
export class ToastComponent implements OnInit {
  ...
  showMsg(msg) {
    ...
    if (msg.data.type != 'chat') {
      this.messageService.add({ key: 'custom', severity: 'info', life: 4500, summary: `${this.translate.instant(`${this.localization.transform('order')}`)} ${this.translate.instant('No.')} ${msg.data.orderId}`, detail: msg.notification.body });
      if(msg.data.extra_product) {
        this.message = msg;
      }
    } else {
      this.message = msg;
      this.messageService.add({ key: 'custom', severity: 'info', life: 4500, summary: 'New Message', detail: msg.notification.body });
    }
  }

  notificationClick() {
    console.log('message', this.message);
    if (this.message.data.type && this.message.data.type == "chat") {
      this.router.navigate(['/account/profile']);
      return false;
    }

    if ((!this.message.data.type || this.message.data.type != "chat") && this.message.data.agent_created_id) {
    ...
    if(this.message.data.extra_product) {
      this.router.navigate(['/orders/order-detail'], {
        queryParams: { orderId: this.message.data.orderId }
      });
    }
    this.messageService.clear();
  }
  rateAgent(orderObj: any) {
    this.messagingService.rateAgent('agent', orderObj?.notification?.body, this.translate.instant('Do you want to rate the Agent'), orderObj)
  }
  onReject() {

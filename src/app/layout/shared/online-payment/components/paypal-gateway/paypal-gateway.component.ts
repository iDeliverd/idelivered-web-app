...
export class PaypalGatewayComponent extends PaymentBaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    var PAYPAL_SCRIPT = `https://www.paypal.com/sdk/js?client-id=${value}&currency=${GlobalVariable.CURRENCY_NAME}`;
      ...
      paypal.Buttons({
        createOrder: () => {
          return new Promise((resolve, reject) => {
            this.httpService.getData(ApiUrl.paypalCreateOrder, { 'net_amount': this.order.amount.toFixed(2), 'currency': GlobalVariable.CURRENCY_NAME })
              .subscribe((res) => {

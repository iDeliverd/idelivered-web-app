...
export class SuppliersComponent implements OnInit, OnDestroy {
  ...
  onCatSortBy(catShortBy: number) {
    ...
    if (catShortBy == 2) {
      this.util.handler.selfPickup = 3;
    }
  ...
  getSuppliers() {
    ...
    if (this.settings.isCustomFlow) {
       this.categoryId = this.route.snapshot.queryParams['cat_flow_id'];
      param_data['categoryId'] = this.categoryId || (this.categories.find(c => c.type == this.settings.app_type)).id
    }

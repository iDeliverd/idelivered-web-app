...
export class PopularProductsComponent implements OnInit, OnDestroy {
  ...
  fetch() {
    const limit = this.settings.show_eight_popular_products ? 8 : 10;
  ...
  addProduct(product) {    
    if (product.adds_on && product.adds_on.length && (this.settings.app_type == 1 || this.settings.enable_services_customize_flow == 1)) {

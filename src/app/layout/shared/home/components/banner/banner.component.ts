...
import { Component, OnInit, HostListener, Inject, AfterViewInit, ViewChild, AfterViewChecked } from "@angular/core";
...
export class BannerComponent implements OnInit,AfterViewChecked {
  ...
  size: string = '1920x1200';
  old_banner: boolean = false;
  ...
  getScreenSize(event?) {
    ...
    // this.size = `${window.innerWidth}x${this.banner_height}`;
  }
      ...
      (settings: AppSettings) => {
        if (settings) {
          ...
          if (this.settings.is_custom_category_template === '1') {
            ...
            this.slideConfig.arrows = (!!this.settings.show_slick_arrows);
  ...
  ngAfterViewChecked(){
    if(this.settings.enable_autoplay_slick){
      this.slideConfig.autoplay = true;
      this.slideConfig.autoplaySpeed = 2000
    }
      setTimeout(()=>{
        if(this.settings.show_slick_arrows){
          this.slickModal['el']['nativeElement']['children'][0].style.left = 0;
          this.slickModal['el']['nativeElement']['children'][0].style.zIndex = 2;
          this.slickModal['el']['nativeElement']['children'][2].style.right = 0;
         }
      },500)

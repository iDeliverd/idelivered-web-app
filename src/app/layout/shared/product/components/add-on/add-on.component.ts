...
export class AddOnComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.pageProducts = this.config.data["pageProducts"];
    ...
    this.addOnItems.forEach(item => {
      ...
      if (item.value) {
        item.value.forEach(type => {
          ...
          if (this.product.is_liquor) {
            if (type.bottle_count) {
              type.bottle_price = type.bottle_count * this.settings.liquor_bottle_tax;
            }
          }
        });
      }
  ...
  calculatePrice() {
    ...
    this.customizePrice = this.settings.remove_price_in_customisation == 1 ? 0 : Number(this.product.fixed_price);
    ...
    this.customizeItems.forEach(item => {
      item.value.forEach(type => {
        ...
        this.product.bottlePrice = Number(type.bottle_count);
        
      });
    });
    console.log('this.customizePrice', this.customizePrice);
  }
  ...
  */
  ...
  updateQuantity(id, val): void {
    if (
      ...
      this.message.toast(
        "info",
        this.translate.instant(`${this.localization.transform('product_inventory_limit')}`)
      );
      return;
    }
    ...
    if (item.quantity <= 0) {
      ...
      if (this.product.customization.length < 1) {
        ...
        this.cartService.addToCart(this.product, this.pageProducts);
        this.totalPrice = this.cartService.calulateProductPrice(this.product);
      }
    } else {
      this.cartService.addToCart(this.product, this.pageProducts);
  ...
  comparsion(): void {
    if (this.product.customization && this.product.customization.length) {
      if(this.settings.enable_services_customize_flow == 1 && this.settings.laundary_service_flow == 0){
  ...
  addToCart(): void {
    ...
    this.addOnItems.forEach(item => {
      ...
      if (!isMinSelect) {
        ...
        if (item.selectedCount < item.value[0].min_adds_on && item.value[0]["is_multiple"] == 1) {
          ...
          this.message.toast(
            "info",
             this.settings.hide_atleast_from_popup ? `${this.translate.instant("Please Select Item In")} ${item.name}` :
             `${this.translate.instant('PLEASE SELECT AT LEAST ')} ${item.value[0].min_adds_on} ${this.translate.instant('ITEMS IN')}
        ...
        if (item.selectedCount > item.value[0].max_adds_on && item.value[0]["is_multiple"] == 1) {
          ...
          this.message.toast(
            "info",
            `${this.translate.instant('You cannot select more than')} ${item.value[0].max_adds_on} ${this.translate.instant('ITEMS IN')} ${item.name}`
          );
          return;
        }
    ...
    this.comparsion();
    if(this.settings.enable_services_customize_flow == 1 && this.settings.laundary_service_flow == 0){
      this.cartService.addToCart(this.product, this.pageProducts,true);
    } else {
      this.cartService.addToCart(this.product, this.pageProducts);
  ...
  getCustomBottlePriceDetail(customize: any) {
    let bottle_price = 0;
    let bottleCount = 0;
    customize.data.forEach(i => {
      bottleCount = i.value
        .map(el => el.bottle_count)
        .toString()
        .replace(/,/g, ", ");

    });
    bottle_price += Number(bottleCount) * this.settings.liquor_bottle_tax * customize.quantity;
  ...
  getCustomePrice(customize: any): number | string {
    ...
    console.log('this.customizePrice 1111', this.customizePrice, ((totalPrice + addOnPrice) * customize.quantity));

...
export class LayoutComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  getScreenSize(event?) {
    ...
    GlobalVariable.IS_MOBILE = (width > 768) ? false : true;
    this.isMobile = GlobalVariable.IS_MOBILE    
  }
  ...
  ngOnInit() {
    ...
    this.settingSubscription = this.util.getSettings.subscribe(
      (settings: AppSettings) => {
        if (!this.settings && settings) {
          ...
          if(this.settings.selected_template == 3 && this.settings.app_type == 8){
            this.body.backgroundColor = "#fff";
            this.style.secondaryColor = "rgba(0,0,0,0.22)";
            this.style.backgroundColor = "#fff";
            this.style.cardBackgroundColor = "#fff";
          }
         
          this.openInstanceSelection(false);
          if (this.settings.enable_website_font_family && this.settings.website_font_family) {
            this.document.body.style.fontFamily = this.settings.website_font_family;
          }
          this.checkUrl();
    ...
    if(this.settings.display_cart_floating_icon == 1) {
  ...
  checkUrl() {
    this.router.events.subscribe((routerEvent: any) => {
      if (routerEvent?.url == '/' || routerEvent?.url == 'cart' || routerEvent?.route?.path == '/' || routerEvent?.route?.path == 'cart') {
        this.topMargin = true;
      }
      else {
  ...
  structuredData() {
    ...
    this.meta.addTag({ property: 'og:image', content: "https://royoorders1.imgix.net/profilePic_1597819425342.png", alt: "scrubble" });

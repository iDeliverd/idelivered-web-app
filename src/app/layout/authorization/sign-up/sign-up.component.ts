...
export class SignUpComponent implements OnInit, OnDestroy {
  ...
  createSinglePageSignUpForm() {
    ...
    if(this.settings.enable_date_of_birth) {
      if(this.settings.require_date_of_birth) {
        this.signUpForm.addControl('dob', new FormControl('', [Validators.required]))
      } else {
        this.signUpForm.addControl('dob', new FormControl(''))
      }
    }
  ...
  createSignUpForm() {
    ...
    if(this.settings.enable_sign_up_form_confirm_password == 1){
      this.signUpForm.addControl('passwordField', this.formBuilder.group({
        password: ['', [Validators.required, Validators.minLength(6), Validators.maxLength(20)]],
        confirmPassword: ['', Validators.required]
      }, { validator: this.checkPasswords }))
    } else{
        this.signUpForm.addControl('password', new FormControl('', [Validators.required, Validators.minLength(6), Validators.maxLength(20)]))
      }
    this.otherLogins.emit(true);
  }
  ...
  onSubmit() {
    ...
    if (!this.is_tnc_checked && this.settings.auth_terms_check == 1) {
      this.message.toast('info', this.translate.instant("Please accept the terms and conditions!"));
      return;
    }
    var payload;
    if (this.settings.enable_signup_phone_only) {
    ...
    if(this.settings.emailVerification == 1){
      payload['verification_url'] = this.window.origin + `/email-verification`;
    }
    ...
    if (this.settings.user_register_flow == 1) {
      if (this.settings.enable_date_of_birth && payload.dob) {
      // && this.dateOfBirth) {
        payload['dateOfBirth'] = moment(payload.dob.toString()).format('YYYY-MM-DD');
      }
    }
        ...
        if (!!response && response.data) {
          ...
          if (!this.settings.enable_signup_phone_only) {
            if (this.settings.user_register_flow == 1) {
              ...
              this.accesstoken = response.data.access_token;
              if(this.settings.emailVerification == 1){
                this.message.alert("info", "Please check your inbox to validate your email ID!");
                return;
              }else{
                this.createPhoneForm();
                this.step2 = true;
    ...
    this.http.postData(ApiUrl.auth.signUp_step2, form_data)
      .subscribe(response => {
        ...
        if (!!response && response.data) {
          ...
          this.otpView = true;
          this.phoneSubmitted = false;
          this.message.toast('success', this.translate.instant('Please enter the verification code sent to your phone number.'));
        }
      }, error => {
        this.showLoader = false;
      });
  ...
  createInfoForm() {
    ...
    if(this.settings.show_saudi_investment_number) {
      this.infoForm.addControl('investmentNumber', new FormControl('', [Validators.required]))
    }
  }

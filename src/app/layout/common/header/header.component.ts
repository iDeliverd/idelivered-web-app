...
export class HeaderComponent implements OnInit, OnChanges, OnDestroy {
    ...
    this.settingsSubscription = this.util.getSettings.subscribe((settings: AppSettings) => {
      if (!!settings) {
        ...
        if (settings.isCustomFlow) {
          ...
          if (!['/', '/terms-and-conditions', '/about', '/privacy-policy', '/faq'].includes(this.router.url) && !(this.router.url).startsWith('/orders') && !flow_data) {
            this.router.navigate(['']);
          } else if (this.router.url !== '/' && !!flow_data && flow_data.name) {
  ...
  searchProduct(searchedVal: string) {
    let searchResults = [];
    this.util.setLocalData('selected_category', this.categories[0], true);
    this.util.setLocalData('last_categoryId', this.categories[0]?.id);
    this.settings.app_type = this.categories[0]?.type;
    if (searchedVal.trim()) {
      ...
      if (this.searchBy === 1) {
        ...
        this.router.navigate(['/products', 'product-listing', searchedVal.replace(/ &/g, '-')], {
          queryParams: {
            app: this.categories[0]?.type, cat_flow_id: this.categories[0]?.id,
            f: this.util.encryptData(this.allParams), searchBy: this.searchBy
          }
        });
      }

...
export class CategoriesComponent implements OnInit, OnDestroy {
        ...
        if (!!response && response.data) {
          ...
          if (this.settings.isCustomFlow && !!selectedCategory) {
            if (selectedCategory.id) {
              ...
              if (this.categories.length <= 0 && response.data) {
                this.categories = response['data'] || [];
              }
            }
            else {
              const category = response['data'].find((c) => c.type == selectedCategory.type);
  ...
  viewProducts() {
    ...
    let seoValue = selectedCatPath.length ? selectedCatPath[selectedCatPath.length - 1].replace(/ &/g, '-') : 'v';
    let param_obj = {};

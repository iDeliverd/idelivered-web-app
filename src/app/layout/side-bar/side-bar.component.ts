...
export class SideBarComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    // if (!check['catId']) {
    //   if (check['filter'] && check['filter'].brand_ids && check['filter'].brand_ids.length) {
    //     check['catId'] = check['filter'].brand_ids[0];
    //   }
    // }
  ...
  getFilterData() {
    this.filterSubscription = this.util.getFilters
      .subscribe(filterData => {
        ...
        this.filters.created_at = filterData.created_at;
      });
  }
  ...
  filterProducts() {
    //SokoLocal min price range fix
    this.filters.min_price_range = +this.filters.min_price_range;
    ...
    this.filters.created_at = this.filterDate ? moment(this.filterDate.toString()).format('YYYY-MM-DD') : '';
  ...
  checkMinInput($event) {
    // console.log('$event', $event, this.filters.min_price_range);
    if (this.filters.min_price_range == 0) {
      this.filters.min_price_range = null;
    }
  }

  checkMaxInput($event) {
    // console.log('$event', $event, this.filters.max_price_range);
    if (this.filters.max_price_range == 50000) {
      this.filters.max_price_range = null;
    }
  }

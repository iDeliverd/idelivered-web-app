...
export class MessagingService {
  ...
  constructor(
    ...
    this.util.getSettings.subscribe(
      (settings: AppSettings) => {
        if (settings) {
          this.settings = settings;
          this.toast_top_center_pos = this.settings.toast_top_center_pos;
        }
      });
  }
  ...
  toast(type: any, title: string) {
    const toast = swal.mixin({
      toast: true,
      position: !this.toast_top_center_pos ? 'top-end' : 'top', // 'top-end', //

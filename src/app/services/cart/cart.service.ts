...
export class CartService {
  ...
  addToCart(product: any, data?: any, buy_now?: boolean) {
    ...
    if (this.cart.length) {
      ...
      } else if (this.settings.enable_different_delivery_status_handling && (this.util.handler.selfPickup != this.getOrderPickType())) {
        this.message.confirm(`${this.translate.instant('Add This')} ${this.localization.transform('product')} ${this.translate.instant('To Cart')}`, `${this.translate.instant("Existing")} ${this.localization.transform('products')} ${this.translate.instant('Will Be Removed From Cart As Your Cart Has')} ${this.localization.transform('products')} ${this.translate.instant('Of Different')} ${this.translate.instant('Delivery Mode')}!`)
          .then(result => {
            if (result.value) {
              this.cart = [];
              this.setOrderPickType(0);
              if (data.length) {
                data.forEach(product => {
                  product['selectedQuantity'] = 0;
                });
              }
              this.setOrderPickType(this.util.handler.selfPickup || 0);
              this.addProductToCart(product);
              if (buy_now) {
                this.router.navigate(['/cart']);
              }
            ...
            if (this.cart[value]['price_type']) {
              if (product['selectedQuantity'] <= 0) {
                ...
                if (product['price_type']) {
                  product.selectedQuantity = product['selectedQuantity'] * this.timeInterval < product.maxHour ? product.selectedQuantity += (product.duration / this.settings.interval) : product.selectedQuantity ;
                  this.cart[value]['selectedQuantity'] = product['selectedQuantity'];
                  if (product.selectedQuantity >= product.quantity ||
                    product.selectedQuantity * this.timeInterval >= product.maxHour) {
  ...
  calulateProductPrice(product) {
    ...
    if (product.customization && product.customization.length) {
      product.customization.forEach(item => {
        ...
        if(!this.settings.remove_price_in_customisation){
  ...
  calulateLiquorProductPrice(product) {
    let netPrice = 0;
    ...
    if (product.customization && product.customization.length) {
      ...
      if (product.customization && product.customization.length) {
        netPrice = this.settings.liquor_bottle_tax * product.selectedQuantity;
      }
      else {
        netPrice = this.settings.liquor_bottle_tax_without_customization * product.selectedQuantity;
      }
    }

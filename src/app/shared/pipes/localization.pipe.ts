...
export class LocalizationPipe implements PipeTransform, OnDestroy {
  ...
  constructor(private util: UtilityService,
    ...
    // this.localization.register_supplier = `${this.translate.instant('Register As')} ${this.transform('supplier', 0, 1)}`;

    this.localization.business_name = `${this.site_name}`;

    this.localization.register_supplier = `${this.translate.instant('Register As')} ${this.translate.instant(`${this.transform('supplier', 0, 1)}`)}`;
    ...
    if (!!this.terminology['english']) {
      ...
      if (this.settings.selected_template == 5) {
        this.localization.choose_supplier = `Choose a ${this.terminology['english'].supplier}`;
      }
  ...
  selectTerminology(type) {
    if (this.settings.isCustomFlow) {
      let selected_category = this.util.getLocalData('selected_category', true);
      let category;
      if (!!selected_category) {
        category = this.categories.find(category => (category.type == type && selected_category.name == category.name));
      } else {
        category = this.categories.find(category => (category.type == type));
      }
